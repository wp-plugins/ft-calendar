					...
					if ( isset( $request['response']['code'] ) && 200 == $request['response']['code'] ) {
						...
						if ( isset( $response->support_status ) && $response->support_status ) {
							...
							if ( isset( $response->support_status->exp_date )  && ! empty( $response->support_status->exp_date ) ) {
								...
								if ( $this->ps_status ) {
									delete_option( 'external_updates-' . $this->plugin_slug );
									$ft_ps_client_auto_update = new FT_Premium_Support_PluginUpdate_Checker( $this->server_url . '?ft-pss-premium-request=1&ft-pss-upgrade-request-product-id=' . $this->product_id . '&ft-pss-upgrade-request-site=' . $this->site_url . '&exp=' . $response->support_status->exp_date, FT_CAL_PATH, $this->plugin_slug, 1 );
								}
